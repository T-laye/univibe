...
export async function GET(request: Request) {
	try {
		...
		const safeRegs = (registrations ?? []) as any[];

		const hostEvents = safeEvents.map((event: any) => ({
			id: event.id,
			title: event.title,
			...
			virtualPlatform: event.virtual_platform ?? null,
...
export async function POST(request: Request) {
	try {
		...
		if (payload.endDate) {
			...
			endDateISO = `${payload.date}T${payload.endTime}:00`;
		}

		const durationInMinutes = endDateISO
		...
		const { data, error } = await db
			...
			.insert({
				...
				total_capacity:
					payload.capacityType === "unlimited" ? 0 : (payload.capacity ?? 0),
				registered_count: 0,
				attended_count: 0,
